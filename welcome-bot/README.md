...
La commande slash `/ticket-panel` s'enregistre **automatiquement** au démarrage du bot (que ce soit en local, sur Railway ou ailleurs) — pas besoin de lancer une commande à part. Avec `GUILD_ID` renseigné dans le `.env`, elle apparaît immédiatement sur Discord ; sans, ça peut prendre jusqu'à 1h (déploiement global).

Tu peux aussi la relancer manuellement à tout moment si besoin :
```bash
npm run deploy
```
